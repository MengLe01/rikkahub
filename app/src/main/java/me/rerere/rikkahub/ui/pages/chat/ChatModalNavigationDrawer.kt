...
internal const val ChatDrawerAnimationDurationMillis = 256
private const val HorizontalDirectionRatio = 1.732f

private fun Offset.isHorizontalGesture(): Boolean =
    abs(x) > abs(y) * HorizontalDirectionRatio
...
internal class ChatDrawerState(initialValue: DrawerValue) {
    ...
    private suspend fun animateTo(targetValue: DrawerValue) {
        ...
        animation.animateTo(
            ...
            animationSpec = tween(
                durationMillis = ChatDrawerAnimationDurationMillis,
        ...
        modifier = modifier
            ...
            .pointerInput(drawerState, drawerWidth, layoutDirection, velocityThreshold) {
                awaitEachGesture {
                    ...
                    while (true) {
                        // Observe after descendants first, matching vertical scrolling: a nested
                        // horizontal scroller may claim the gesture before the drawer does.
                        val event = awaitPointerEvent(PointerEventPass.Main)
                        val change = event.changes.firstOrNull { it.id == down.id }
                        if (change == null) {
                            if (direction == GestureDirection.DrawerHorizontal || interruptedAnimation) {
                                settleScope.launch {
                            ...
                            when (direction) {
                                GestureDirection.Undecided -> {
                                    if (totalDelta.getDistance() > viewConfiguration.touchSlop) {
                                        val isHorizontal =
                                            totalDelta.isHorizontalGesture()
                                        direction = when {
                                            !isHorizontal -> GestureDirection.Vertical
                                            change.isConsumed -> GestureDirection.ChildHorizontal
                                            abs(totalDelta.x) > viewConfiguration.touchSlop -> {
                                                GestureDirection.DrawerHorizontal
                                            }

                                            else -> GestureDirection.Undecided
                                        }

                                        if (direction == GestureDirection.DrawerHorizontal) {
                                            drawerState.dragBy(
                                ...
                                GestureDirection.ChildHorizontal,
